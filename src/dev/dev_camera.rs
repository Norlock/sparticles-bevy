...
fn pan_orbit_camera(
    ...
    let scroll = ev_scroll.iter().fold(0., |acc, item| acc + item.y);
    ...
    for (mut pan_orbit, mut transform, projection) in query.iter_mut() {
        ...
        let mut any = false;
        if 0. < rotation_move.length_squared() {
            any = true;
            let window = get_primary_window_size(&windows);

            let delta = rotation_move.x / window.x * PI * 2.;
            let delta_x = if pan_orbit.upside_down { -delta } else { delta };
...
fn spawn_camera(mut commands: Commands) {
    let translation = Vec3::new(0., 0., 100.);
