...
impl LooseMovementAnimation {
    fn apply_inverse_gravitational_force(&mut self, data: &mut EmitterData) {
        let gravitational_point = self.base;
        ...
        let x_distance = gravitational_point.x - emitter_center_x;
        let x_pull_distance_abs = self.range - x_distance.abs().min(range_delta);
        let x_pull_distance = x_pull_distance_abs * x_distance.signum();

        let y_distance = gravitational_point.y - emitter_center_y;
        let y_pull_distance_abs = self.range - y_distance.abs().min(range_delta);
        let y_pull_distance = y_pull_distance_abs * y_distance.signum();

        let z_distance = gravitational_point.z - emitter_center_z;
        let z_pull_distance_abs = self.range - z_distance.abs().min(range_delta);
        let z_pull_distance = z_pull_distance_abs * z_distance.signum();
